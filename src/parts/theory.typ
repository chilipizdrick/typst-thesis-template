...
древесная структура с произвольным количеством дочерних узлов в каждом узле. Для
реализации большинства алгоритмов построения суффиксного дерева необходимо
добавление в конец изначального слова защитного символа, не входящего в "алфавит",
на котором построено указанное слово (Рисунок @abbabac).
...
#figure(
  table(
    ...
    [$O(n)$], [Алгоритм Укконена], [$O(n)$], [$O(n^2)$#footnote(
        [Только в случаях с большим изначальным алфавитом и обработке нерегулярных слов.],
      )],
...
суффиксную ссылку на себя (необходимо для оптимальной работы алгоритма), а также
хеш-таблицу дочерних узлов по их уникальным идентификаторам (выбор хеш таблицы
обусловлен возможностью добавления элемента и доступа к нему с временной
сложностью в $O(1)$). Используемый алгоритм построения дерева --- алгоритм
Укконена. Данный выбор обусловлен возможностью реализовать наиболее
универсальный класс суффиксного дерева, в том числе способный работать в режиме _online_.
